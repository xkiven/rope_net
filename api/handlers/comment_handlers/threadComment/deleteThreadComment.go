...
func DeleteThreadComment(c *gin.Context) {
	commentIDStr := c.Param("threadCommentID")
	var commentID uint
	...
	result := db.Where("id = ?", commentID).First(&threadComment)
	if result.Error != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"status": 10003,
			"info":   "查找失败",
	...
	if threadComment.UserID != userID {
		c.JSON(http.StatusInternalServerError, gin.H{
			"status": 10004,
			"info":   "你不是此评论的主人",
		})
	...
	//数据库中删除
	logger.Info("在数据库中删除")
	result = db.Delete(&models.ThreadComments{}, threadComment.ID)
	if result.Error != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"status": 10005,
			"info":   "删除失败",
